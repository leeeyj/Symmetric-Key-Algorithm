...
uint8 m0d(uint8 a){
	return m08(a) ^ m04(a) ^ a; 
}

uint8 m0e(uint8 a){
	return m08(a) ^ m04(a) ^ m02(a); 
}

void AddRoundKey(uint8* arr, uint8* key, uint8 Round, uint8 flag){
	if(flag == 0){
        for(uint8 i = 0; i < 16; i++){arr[i] = arr[i] ^ key[Round*16 + i];}
    }
    if(flag == 1){
        uint8 RoundKey[16];
        for(uint8 i = 0; i < 16; i++){RoundKey[i] = key[Round*16 + i];}
        InvMixColumns(RoundKey);
        for(uint8 i = 0; i < 16; i++){arr[i] = arr[i] ^ RoundKey[i];}
    }
}
...
void InvShiftRows(uint8* arr){
	...
	temp = arr[1];
	arr[1] = arr[13]; arr[13] = arr[9];
	arr[9] = arr[5];  arr[5] = temp;
	...
	temp = arr[3];
	arr[3] = arr[7];   arr[7] = arr[11];
	arr[11] = arr[15]; arr[15] = temp;
}

void ShiftSubMix(uint8* arr){
    uint32 a[4];
    for(uint8 i = 0; i < 4; i++){
        a[i] = Te0[arr[i*4]] ^ Te1[arr[(i*4+5)%16]] ^ Te2[arr[(i*4+10)%16]] ^ Te3[arr[(i*4+15)%16]];
    }
    
    for(uint8 i = 0; i < 4; i++){
        for(uint8 j = 0; j < 4; j++){
            arr[i*4+j] = a[i] >> (24 - 8 * j) & 0xff; 
        }
    }    
}   
...
void InvShiftSubMix(uint8* arr){
    uint32 a[4];
    for(uint8 i = 0; i < 4; i++){
        a[i] = Td0[arr[i*4]] ^ Td1[arr[(i*4+13)%16]] ^ Td2[arr[(i*4+10)%16]] ^ Td3[arr[(i*4+7)%16]];
    }
...
void InvMixColumns(uint8* arr){
	for(uint8 i = 0; i < 4; i++){
		...
		for(uint8 j = 0; j < 4; j++){
			if(j == 0) arr[i*4+j] = m0e(a) ^ m0b(b) ^ m0d(c) ^ m09(d);
			if(j == 1) arr[i*4+j] = m09(a) ^ m0e(b) ^ m0b(c) ^ m0d(d);
			if(j == 2) arr[i*4+j] = m0d(a) ^ m09(b) ^ m0e(c) ^ m0b(d);
			if(j == 3) arr[i*4+j] = m0b(a) ^ m0d(b) ^ m09(c) ^ m0e(d);
		}
	}
}
...
void AES_Encryption(uint8* plain, uint8* key, uint8 keySize, uint8* cipher){
	...
	AddRoundKey(cipher, RoundKey, 0, 0);
	for(uint8 i = 1; i < Round; i++){
		ShiftSubMix(cipher);
		AddRoundKey(cipher, RoundKey, i, 0);
	}
    ShiftRows(cipher);
	SubBytes(cipher);
	AddRoundKey(cipher, RoundKey, Round, 0);
...
void AES_Decryption(uint8* cipher, uint8* key, uint8 keySize, uint8* plain){
	...
	for(uint8 i = 0; i < 16; i++){plain[i] = cipher[i];}

	uint8* RoundKey = KeySchedule(key, keySize);

	AddRoundKey(plain, RoundKey, Round, 0);
	for(uint8 i = Round-1; i >= 1; i--){
		InvShiftSubMix(plain);
		AddRoundKey(plain, RoundKey, i, 1);
	}
	InvShiftRows(plain);
	InvSubBytes(plain);
	AddRoundKey(plain, RoundKey, 0, 0);
